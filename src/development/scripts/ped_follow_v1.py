...
def movebase_client():
   # Create an action client called "move_base" with action definition file "MoveBaseAction"
    ...
    # Select a random pedestrian:
    data = rospy.wait_for_message("/pedsim_simulator/simulated_agents", AgentStates)
    total_ped = len(data.agent_states)
    n_loop = 100
    t_delay = 1
    ped_n = random.randint(1,total_ped)
    print("Delay: %d | Iterations: %d | Pedestrian: %d" % (t_delay, n_loop, ped_n-1))
    # Follow the pedestrian n_loop times:
    for i in range(n_loop):
        # Pedestrian position:
        ped = rospy.wait_for_message("/pedsim_simulator/simulated_agents", AgentStates)
        ped_x = ped.agent_states[ped_n-1].pose.position.x
        ped_y = ped.agent_states[ped_n-1].pose.position.y
        ...
        print("%d | Ped: %d | Goal: (%.2f, %.2f) | Dist: %.2fm" % (i, ped_n-1, ped_x, ped_y, dist))
