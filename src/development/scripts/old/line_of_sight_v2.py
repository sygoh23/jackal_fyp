...
while True:
    ...
    for k in range(len(ped_x_in_range)):
        ...
        print("Pedestrian %d | Score: %d / %d" % (ped_num_in_range[k], len(cloud_x_in_boundary), len(cloud_x)))

        if (len(cloud_x_in_boundary) < 10):
    ...
    plt.scatter(ped_x_los, ped_y_los, c='g', marker='.', s=25)
